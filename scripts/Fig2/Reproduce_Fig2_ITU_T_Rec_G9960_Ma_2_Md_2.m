...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %%
addpath('../../data/Fig2')
load('Fig2_Parameters_for_ITU_T_Rec_G9960_Ma_2_Md_2.mat');
...
PSD_max = max(max(y_h(find(x>Pd_todas(end)+N/2+1))), max(y_h(find(x<Pd_todas(1)+N/2-1))));
x_PSD_max = x(find(y_h==PSD_max));

% Fig 2 (ITU-T Rec. G9960 Ma=Md=2)
figure,
plot(x(indices),y_h(indices), 'DisplayName', 'h_k^{t-e}(M_a=M_d=2)', 'Color', "#4DBEEE"); hold on; grid on;
