...
% Fig 1 (a)
figure,
plot(x(indices),y_h(indices), 'DisplayName', '|H_k(f)|'); hold on; grid on;
plot(x(indices),y_p(indices), 'DisplayName', '|P_k(f)|');
...
% Fig 1(b)
figure,
plot(x,y_h, 'DisplayName', '|h_k(n)|'); grid on; hold on;
plot(x,y_p, 'DisplayName', '|p_k(n)|');
